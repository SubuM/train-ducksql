...
if st.session_state.get("token"):
    ...
    if not is_admin:
        ...
        # Execute query
        if sql_query.strip() and sql_query != st.session_state.get("last_executed_sql_user"):
            result = run_sql_query(username, sql_query)
            st.session_state["last_executed_sql_user"] = sql_query
            st.session_state["query_history"].setdefault(username, []).append(sql_query)
        ...
        # Query History
        if st.session_state["query_history"].get(username):
            st.subheader("Query History")
            for i, q in enumerate(reversed(st.session_state["query_history"][username]), 1):
    ...
    # ---------------- Admin Dashboard ----------------
        ...
        st.write("Manage all users and run queries on their DuckDB databases.")
        ...
        if selected_user:
            ...
            selected_theme = st.selectbox("Select ACE Editor Theme for Admin Query", ace_themes, index=0, key="admin_theme")

            # Admin SQL Editor
            admin_sql_query = st_ace(
                value="",
                language="sql",
                theme=selected_theme,
                height=300,
                ...
                show_print_margin=False,
                wrap=True,
                placeholder=f"Write SQL query for `{selected_user}` here..."
            )

            # Execute admin query
            if admin_sql_query.strip() and admin_sql_query != st.session_state.get(f"last_executed_sql_{selected_user}"):
                result = run_sql_query(selected_user, admin_sql_query)
                st.session_state[f"last_executed_sql_{selected_user}"] = admin_sql_query
                st.session_state["query_history"].setdefault(selected_user, []).append(admin_sql_query)
            ...
            if st.session_state["query_history"].get(selected_user):
                st.subheader(f"Query History for `{selected_user}`")
                for i, q in enumerate(reversed(st.session_state["query_history"][selected_user]), 1):
                    st.code(f"{i}: {q}", language="sql")

            # Tables and schema
            user_tables = list_tables(selected_user)
            if user_tables:
                table_to_preview = st.selectbox("Select Table to Preview", user_tables, key="admin_table_select")
                if table_to_preview:
